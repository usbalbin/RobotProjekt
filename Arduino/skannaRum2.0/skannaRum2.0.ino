...
void skrivTillKarta(){
  ...
  for(avst; avst > 0;avst-=intervall){
    ...
    x=exakt;
    if(x>exakt-tolerans&&x<exakt+tolerans){
      float exakt = (float)avst*cos((((float)vinkel * 71) / 4068));
      y=exakt;
      if(y>exakt-tolerans&&x<exakt+tolerans){
...
void riktning(int onskadVinkel, int kanslighet){
  ...
  deltaVinkel*=1-(vinkelHastighet/kanslighet);
  hastighetLeft-=deltaVinkel;
  hastighetRight+=deltaVinkel;
  
  if(hastighetLeft<0){
    hastighetLeft=0;
  }
  if(hastighetRight>255){
    hastighetRight=255;
  }
    
  else{              //Vrida vänster höger
